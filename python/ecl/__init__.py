...
ECL_LEGACY_INSTALLED = not os.path.isdir(
    os.path.join(os.path.dirname(__file__), dll_dir)
)
...
if not ECL_LEGACY_INSTALLED:
    ...
    def dlopen_libecl():
        ...
        if platform.system() == "Linux":
            ...
            os.environ["ECL_SKIP_SIGNAL"] = "True"
            # check for zlib
            try:
                zlib_path = os.path.join(sys.exec_prefix, "Library", "bin", "zlib.dll")
                ctypes.OleDLL(zlib_path)
            except ImportError:
                raise ImportError(f"Missing zlib.dll could not be found {zlib_path}")
        else:
            raise NotImplementedError("Invalid platform")

        return ctypes.CDLL(path, ctypes.RTLD_GLOBAL)  # , winmode=winmode)
